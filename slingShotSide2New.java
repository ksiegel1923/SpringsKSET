...
public class slingShotSide2New extends AbstractSimulation{
...
//					System.out.println("Left Down");
	...
	public void initialize() {
		...
		for (double i = 0; i < Math.floor(numberSprings/2)+1; i=i+1) {
			//declaring a new particle for each of the springs
			if(i==Math.floor(numberSprings/2)) {
				particle2D p = new particle2D(k, mBall, l, 0, 0, 0, 0, 50, 2);
				//				p.color=Color.blue;
				frame.addDrawable(p); //adds particles to the displayframe
				...
				particle2D p = new particle2D(k, m, l, 0, 0, 0, 0, i/5+40, ((-i/10)+5)+2);
				frame.addDrawable(p); //adds particles to the displayframe
				springRight1.add(p); //adds particles to the spring arraylist
				p.setXY(p.getPx(), p.getPy()); 
			}
		...
		for (double i = 0; i < Math.floor(numberSprings/2)+1; i=i+1) {
			if(i==Math.floor(numberSprings/2)) {
				particle2D p = new particle2D(k, mBall, l, 0, 0, 0, 0, 50, 0+2);
				//				p.color=Color.blue;
				frame.addDrawable(p); 
				springRight2.add(p); 
				p.setXY(p.getPx(), p.getPy()); 
	...
	public static void main(String[] args) {
		SimulationControl.createApp(new slingShotSide2New()); //runs the simulation
	}
}